			...
			if ([token length] > 0) {
				...
				if ([scanner isAtEnd]) {
					[NSException raise:L4InvalidSpecifierException 
								format:@"Expected a valid specifier character at position %ld in the string '%@'",
					 (unsigned long)[scanner scanLocation], [scanner string]];
						...
						if ([scanner isAtEnd]) {
							...
							if ([scanner isAtEnd]) {
								[NSException raise:L4InvalidBraceClauseException 
											format:@"Expected a closing brace '}' character at position %ld in the string '%@'",
								 (unsigned long)[scanner scanLocation], [scanner string]];
							} else if ([[scanner string] characterAtIndex:[scanner scanLocation]] == (unichar)'}') {
								[token appendString:tempString];
								[token appendFormat:@"%C", [[scanner string] characterAtIndex:[scanner scanLocation]]];
					...
					switch (specifierChar) {
							...
							if (([scanner scanLocation] + charsToSkip) < [[scanner string] length] && 
								...
								if ([scanner scanUpToString:@"}" intoString:&tempString2]) {
									...
									if (componentLength > 0) {
										...
										[NSException raise:L4InvalidBraceClauseException 
													format:@"Expected a nonzero positive integer at position %ld in conversion specifier %@.", 
										 (unsigned long)([scanner scanLocation] - [tempString2 length]), [scanner string]];
									}
						...
						case 'd':
						{
							...
							if (([scanner scanLocation] + charsToSkip) < [[scanner string] length] && 
								...
								if ([scanner scanUpToString:@"}" intoString:&tempString2]) {
									tempString = [[logEvent timestamp] descriptionWithCalendarFormat:@"%H:%M:%S %Z"
																							timeZone:nil
